...
pub struct Config {
	...
	///
	/// The default behaviour for conduwuit is to attempt to query trusted key
	/// servers before querying the individual servers. This is done for
	/// performance reasons, but if you would like to query individual servers
	/// before the notary servers configured below, set to
	///
	/// (Currently, conduwuit doesn't support batched key requests, so this list
	/// should only contain Synapse servers) Defaults to `matrix.org`
